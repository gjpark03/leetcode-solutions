/*
  ...
  types of candies while still following the doctor's advice. Given the integer array candyType 
...
public:
    int distributeCandies(vector<int>& candyType) {
        ...
        for(int i = 0; i < candyType.size(); i++) {
            typeCount[candyType[i]]++;
        }
        return min((int)typeCount.size(), (int)(candyType.size() / 2));
    }
};
